...
public class DefaultReviewService implements ReviewService {
    ...
    private final DefaultJwtSecurityTokenService defaultJwtSecurityTokenService;

    public Review addReview(ReviewRequest reviewRequest, String authorizationHeader) {
        String token = extractToken(authorizationHeader);

        User user = userService.findByEmail(defaultJwtSecurityTokenService.extractUsername(token));
